...
<<<<<<< HEAD
  ...
  useEffect(() => {
    FetchAPIService(`https://disease.sh/v3/covid-19/countries`)
      .then((result) => {
  ...
  const Filter = (event) => {
    sortedRecords((val) =>
      countriesData.filter((f) =>
        f.country.toLowerCase().includes(event.target.value)
      )
    );
  };
    ...
    <div className={props.className}>
      ...
      <ul className="dashboard__countrylist--list">
        {records.map((country, index) => {
          return <CountryItem key={country.id + index} countryData={country} />;
        })}
      </ul>
    </div>
  );
